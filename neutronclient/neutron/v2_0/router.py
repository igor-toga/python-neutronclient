...
class AddGwInterfaceRouter(neutronV20.NeutronCommand):
    """Set the external network gateway for a router."""

    resource = 'router'

    def get_parser(self, prog_name):
        parser = super(AddGwInterfaceRouter, self).get_parser(prog_name)
        parser.add_argument(
            'router', metavar='ROUTER',
            help=_('ID or name of the router.'))
        parser.add_argument(
        ...
        parser.add_argument(
            'l3_agent',
            metavar='L3_AGENT',
            help=_('ID of L3 agent.'))
        parser.add_argument(
            '--alt-gateway-ip', metavar='ip_address=IP_ADDR',
            help=_('External address for gateway port on host '
                   'where L3 agent resides'))
        parser.add_argument(
            '--fixed-ip', metavar='subnet_id=SUBNET,ip_address=IP_ADDR',
            action='append',
            type=utils.str2dict_type(optional_keys=['subnet_id',
    ...
    def take_action(self, parsed_args):
        ...
        router_dict['alt_gateway_ip'] = parsed_args.alt_gateway_ip
        if parsed_args.fixed_ip:
            ips = []
            for ip_spec in parsed_args.fixed_ip:
                subnet_name_id = ip_spec.get('subnet_id')
                ...
                ips.append(ip_spec)
            router_dict['external_fixed_ips'] = ips
        neutron_client.add_gateway_interface(_router_id, router_dict)
        print(_('Add gateway interface for router %s') % parsed_args.router,
              file=self.app.stdout)


class RemoveGwInterfaceRouter(neutronV20.NeutronCommand):
    """Remove an external network gateway from a router."""

    resource = 'router'

    def get_parser(self, prog_name):
        parser = super(RemoveGwInterfaceRouter, self).get_parser(prog_name)
        parser.add_argument(
            'router', metavar='ROUTER',
            help=_('ID or name of the router.'))
        parser.add_argument(
